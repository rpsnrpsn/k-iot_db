...
# 1. 트리거 사용 목적
-- 일정 작업 자동화: 일련의 동작을 같이 처리 (회원가입 시 가입 로그 자동 기록)
...
# - 재고 수량 자동 조정, 포인트 자동 적립 등 '자동화 된 비지니스 로직 처리'

# cf) 주의점
# : 남용 시 디버깅 어렵고, 성능 저하 가능성 존재 >> 꼭 필요한 곳에서만 사용

# 6. 트리거 기본 문법
# : 스포어드 프로시저 유사 (call 문 사용 x)
...
## 트리거(trigger) VS 트랜잭션(transction)
# 1) 트리거
# : 이벤트 발생 시 '자동 처리'
