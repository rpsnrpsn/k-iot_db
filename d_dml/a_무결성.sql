...
# 		, 참조값이 반드시 존재해야 함을 뜻함
# 	- 외래 키(Foreiegn Key)를 통해 구현
# 	ex) 학생 - 수강, 고객 - 주문, 게시글 - 댓글 ('해당 데이터'에서 - '값'을 참조)

# 3) 도메인 무결성
#	: 테이블의 각 필드(열, 컬럼)가 정의된 도메인에 맞는 값을 가져야 함을 뜻함
# 	- 데이터타입, 포멧, 범위 등이 적절하게 유지
# 	- 데이터타입 지정, unsigned, 제약 조건(CHECK) 등을 통해 구현
# 	ex) 은행 계좌에 입금되는 금액 - 양의 정수만 허용(unsigned)
# 		점수에 입력되는 숫자 - 0에서 100사이의 점수(CHECK)

# cf) 도메인(domain)
#	: '데이터타입, 길이, 가능한 값의 범위'로 구성

# 4) 사용자 정의 무결성
# : 특정 애플리케이션의 규칙에 딸 ㅏ정의
