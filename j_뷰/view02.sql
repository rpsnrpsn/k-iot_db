...
# 'v_view_test02', 'CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_view_test02` 
...
# - 단, 뷰에 푸함되지 않은 컬럼에 대한 제약조건이 있을 경우 제한 될 수 있음 (권장 x)

# member 테이블 mem_id, mem_name(필수값, NOT NULL), addr 등 존재
# > member 테이블로 생성한 v_member 뷰는 mem_idm addr만 존재
