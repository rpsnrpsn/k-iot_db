...
#	: 공백이나 특수문자가 포함된 이름 사용 (명명 규칙 위반이지만,,)
SELECT `student name` FROM `busan high school`;

# >> 혹시 모를 예약어와의 충돌이나
#		, 외부 시스템 연동 시 문제 방지를 위해 백틱을 사용하는 것이 안정적

# 2) 따옴표
# 	: 문자열 데이터를 저장하는 데 사용 (문자열 리터럴 표현)
# 	- DB, 테이블, 컬럼명으로는 사용하지 않는 것을 권장
# 	- 실제 데이터임을 명시
SELECT 'price' FROM `products`; -- 가격 컬럼이 아니라 실제 price 문자열(글자)로 인식
